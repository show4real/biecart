...
class Senangpay extends PaymentGatewayBase implements RecurringSupport
{
    ...
    public function charge_customer_recurring(array $args)
    {
        ...
        $params = http_build_query([
            'order_id' => 'payment_type_###'.$args['payment_type'].'##'.XgPaymentGateway::wrapped_id($args['order_id']),
            'recurring_id' => $this->getRecurringId(),
            'hash' => $this->getHashForRecurringPayment($args['order_id']),
            'name' => $args['name'],
            'email' => $args['email'],
            'phone' => $args['phone'] ?? " ",
            'amount' => $args['amount']
    ...
    private function getHashForRecurringPayment(mixed $order_id)
    {
        return match ($this->getHashMethod()) {
            'md5' => $this->getMd5HashForRecurringPayment($order_id),
            default => $this->getSha256HashForRecurringPayment($order_id)
        };
    }

    private function getMd5HashForRecurringPayment(mixed $order_id)
    {
        return md5($this->getSecretKey() . $this->getRecurringId() . $order_id);
    }

    private function getSha256HashForRecurringPayment(mixed $order_id)
    {
        return hash_hmac('sha256', $this->getSecretKey() . $this->getRecurringId() . $order_id, $this->getSecretKey());
    }
