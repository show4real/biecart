...
class MobileCheckoutServices
{
    ...
    public static function getCartProducts($validated_data): array
    {
        $cartArr = [];
        $cart = json_decode($validated_data['cart']);
    ...
    public function getTotalPriceDetails($validated_data)
    {
        $total = 0.0;
        $cartArr = self::getCartProducts($validated_data);
    ...
    public function createOrder($validated_data, $user)
    {
        ...
        $totalPriceDetails = $this->getTotalPriceDetails($validated_data);
